...
// Returns a half-vector sampled according to the GGX distribution
...
void main() {
    // v_UV.x = NdotV, v_UV.y = roughness
    // Clamp NdotV to avoid edge artifacts
    float NdotV = max(v_UV.x, 0.001);
    float roughness = max(v_UV.y, 0.001);
